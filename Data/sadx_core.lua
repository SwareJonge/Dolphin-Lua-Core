...
local smoothTurnIncrement = 4000
core.smoothTurnIncrement = smoothTurnIncrement

----- GETTING DATA FROM MEMORY -----

local function getXPos()
	local pointer = 0x7A8280
	local offset = 0x20
...
-- smoothTurn: limits the given angle to a range of +smoothTurnIncrement and -smoothTurnIncrement units around current Y rotation angle
local function smoothTurn(angle)

	if (angle - getYRot()) % 65536 > smoothTurnIncrement and (angle - getYRot()) % 65536 < 65536 / 2 then
		angle = getYRot() + smoothTurnIncrement
	end
	
	if (angle - getYRot()) % 65536 >= 65536 / 2 and (angle - getYRot()) % 65536 < 65536 - smoothTurnIncrement then
		angle = getYRot() - smoothTurnIncrement
	end
